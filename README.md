...
| [avaje-metrics](avaje-metrics/SKILL.md) | Avaje Metrics — timers, counters, JVM metrics, @Timed enhancement, OpenTelemetry, Prometheus, StatsD, Graphite |
...
- **Adding metrics, method timing, or metrics export?** → Use `avaje-metrics`
...
ln -sf ~/.agents/avaje-skills/avaje-metrics ~/.agents/skills/avaje-metrics
```
...
cp -r /tmp/avaje-skills/avaje-metrics ~/.agents/skills/avaje-metrics
...
You should see `avaje-nima`, `avaje-inject`, `avaje-config`, `avaje-jsonb`,
`avaje-simple-logger`, and/or `avaje-metrics` listed.
...
avaje-simple-logger, avaje-metrics).
